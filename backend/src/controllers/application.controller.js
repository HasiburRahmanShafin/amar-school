...
const { verifyUploadcareUrl } = require('../services/uploadcare.service');
...
exports.submitApplication = async (req, res) => {
  try {
    const {
      circularId, studentName, dateOfBirth, gender, guardianName, guardianPhone,
      guardianEmail, address, previousSchool,
      photoUrl, birthCertificateUrl, reportCardUrl,
    } = req.body;
    ...
    if (!photoUrl || !birthCertificateUrl) {
      return res.status(400).json({
        success: false,
        message: 'Applicant photo and birth certificate are required',
      });
    }

    // ASSUMPTION: circular.classOrGrade holds a value like "Class 1" for
    // first-graders. Adjust the comparison below if your data uses a
    // different format (e.g. "1", "Grade 1").
    const isClassOne = circular.classOrGrade === 'Class 1';

    if (!isClassOne && !reportCardUrl) {
      return res.status(400).json({
        success: false,
        message: 'Previous year report card is required (not required for Class 1)',
      });
    }

    // This is a PUBLIC, unauthenticated endpoint — verify the uploaded files
    // actually exist on Uploadcare before trusting the urls.
    const [photoValid, birthCertValid] = await Promise.all([
      verifyUploadcareUrl(photoUrl),
      verifyUploadcareUrl(birthCertificateUrl),
    ]);
    if (!photoValid) {
      return res.status(400).json({ success: false, message: 'Applicant photo file could not be verified' });
    }
    if (!birthCertValid) {
      return res.status(400).json({ success: false, message: 'Birth certificate file could not be verified' });
    }
    if (!isClassOne) {
      const reportCardValid = await verifyUploadcareUrl(reportCardUrl);
      if (!reportCardValid) {
        return res.status(400).json({ success: false, message: 'Report card file could not be verified' });
      }
    }
    ...
    const applicant = await Applicant.create({
      ...
      photoUrl, birthCertificateUrl,
      reportCardUrl: isClassOne ? undefined : reportCardUrl,
    });
