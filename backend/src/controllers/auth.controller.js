...
const getMe = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.id)
      .populate('school')
      // Parents need their children's names (not just ids) to populate
      // the "which child" selector on the attendance history page.
      .populate('parentInfo.children', 'name studentInfo.className studentInfo.section');
    if (!user) {
    ...
    res.json({
      ...
      school: user.school
        ? { id: user.school._id, name: user.school.name, status: user.school.status }
        : null,
      // Attendance module needs to know a student's class/section and a
      // teacher's assigned classes right after login/refresh.
