...
// Extracts the leading number from a class label like "Class 1" or "1" -> 1
function getClassNumber(classOrGrade) {
  const match = String(classOrGrade).match(/\d+/);
...
exports.submitApplication = async (req, res) => {
  try {
    const {
      circularId, studentName, dateOfBirth, gender, guardianName,
      guardianPhone, guardianEmail, address, previousSchool,
    ...
    // Photo and birth certificate are always required
    ...
    // Report card is required for every class except Class 1
    const classNumber = getClassNumber(circular.classOrGrade);
    const isClassOne = classNumber === 1;
    if (!isClassOne && !reportCardUrl) {
      return res.status(400).json({
        success: false,
        message: 'Previous year report card is required for this class',
      });
    }
    ...
    const applicant = await Applicant.create({
      ...
      photoUrl, birthCertificateUrl, reportCardUrl: reportCardUrl || undefined,
    });
