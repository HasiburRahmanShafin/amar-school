...
export default function StudentList() {
  ...
  // Provisions a login for a student so they can sign in to Attendance
  // History / Fee Payment. Mirrors the teacher "Create login" flow.
  const handleCreateLogin = async (e, student) => {
    e.preventDefault();
    e.stopPropagation();
    if (!student.guardianEmail) {
      showToast?.('Add a guardian email to this student first', 'error');
      return;
    }
    setAccountBusyId(student._id);
    try {
      const res = await api.post(`/students/${student._id}/account`, {});
      showToast?.(`Login created for ${student.name}. Credentials emailed to ${res.data.email}.`, 'success');
      fetchStudents();
    } catch (err) {
        ...
        <div className={`border rounded-2xl overflow-hidden divide-y ${cardBg} ${divider}`}>
          ...
          {!loading && students.map((s) => (
            ...
            >
              ...
              {s.userId ? (
                <span className={`text-xs px-2 py-0.5 rounded-full ${isDark ? 'bg-indigo-900/40 text-indigo-300' : 'bg-indigo-50 text-indigo-600'}`}>
                  🔑 Login
                </span>
              ) : (
                <button
                  onClick={(e) => handleCreateLogin(e, s)}
                  disabled={accountBusyId === s._id}
                  className={`text-xs font-medium px-2.5 py-1 rounded-lg border disabled:opacity-50
                    ${isDark ? 'border-gray-600 text-gray-200 hover:bg-gray-800' : 'border-blue-300 text-blue-700 hover:bg-blue-50'}`}
                >
                  {accountBusyId === s._id ? 'Creating…' : 'Create login'}
                </button>
              )}
            </Link>
          ))}
