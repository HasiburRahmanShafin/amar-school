...
export default function ManageApplicants() {
  ...
  const fetchApplicants = () => {
    ...
    api.get(`/admissions/applicants${query}`).then((res) => setApplicants(res.data)).finally(() => setLoading(false));
  };
    ...
    <div className="min-h-screen bg-blue-50 py-8 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        {!loading && applicants.length === 0 && (
          <div className="bg-white border border-blue-100 rounded-xl p-8 text-center text-blue-500">No applicants found.</div>
        )}
            ...
            <div key={a._id} className="bg-white border border-blue-100 rounded-xl p-5">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {typeof a.circularId === 'object' ? a.circularId.title : ''} · Guardian: {a.guardianName} ({a.guardianPhone})
                  </p>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-blue-50">
                {['pending', 'reviewed', 'approved', 'rejected'].map((s) => (
                  <button
                    ...
                    className={`text-xs font-medium px-3 py-1.5 rounded-lg border transition-colors ${
                      a.status === s ? 'bg-blue-700 text-white border-blue-700' : 'border-blue-200 text-blue-700 hover:bg-blue-50'
                    }`}
                  >
                    {s}
                  </button>
                ...
                {(a.status === 'approved' || a.status === 'rejected') && !a.resultPublished && (
                  <button onClick={() => publishResult(a._id)} className="ml-auto bg-blue-700 text-white text-xs font-medium px-4 py-1.5 rounded-lg hover:bg-blue-800">
                    Publish result
                  </button>
                )}
                {a.resultPublished && <span className="ml-auto text-xs text-blue-500 font-medium">Result published</span>}
              </div>
            </div>
          ))}
        </div>
